...
void TraceImageCall(TraceData& data, uint32_t yMin, uint32_t yMax, uint32_t xMin, uint32_t xMax, std::promise<int> && p)
{
	...
	for (uint32_t y = yMin; y < yMax; ++y)
	{
		...
		for (int x = xMin; x < xMax; ++x)
		{
			...
			for (int s = 0; s < data.samplesPerPixel; s++)
			{
				...
				Ray r = camera.GetRay(u, v, rngState);
...
static void TraceImage(TraceData& data)
{
	...
	auto widthStep = data.screenWidth / ThreadCount;
	auto heightStep = data.screenHeight / ThreadCount;
	std::promise<int> p;
	auto f = p.get_future();
	for (int i = 0; i < ThreadCount; ++i)
	{
		...
		auto xMin = i * widthStep;
		auto xMax = (i + 1) * widthStep;
		uint32_t yMin = i * heightStep;
		uint32_t yMax = (i + 1) * heightStep;
		threads[i] = std::thread([&, yMin, yMax] {
			TraceImageCall(data, yMin, yMax, 0, data.screenWidth, std::move(promises[i]));
		});

		//std::move(std::thread(TraceImageCall, ));
	}
	//TraceImageCall(data, 0, data.screenHeight, 0, data.screenWidth, std::move(p));
	for (auto& th : threads)
	{
		if (th.joinable())
			th.join();
